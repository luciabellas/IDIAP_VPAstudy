...
embarazos <- embarazos %>% select(id_emb, dinici, dfi) %>%
  ...
  filter(cohort_start_date > as.Date("2011-01-01") &
           cohort_start_date < as.Date("2020-06-01") &
           cohort_end_date > as.Date("2011-01-01") &
           cohort_end_date < as.Date("2020-06-01"))

valproato <- farmacos %>% 
  filter(grepl("N03AG01", cod)) %>% 
...
DBI::dbWriteTable(con, "valproato", valproato, overwrite = TRUE)
...
cdm <- cdmFromCon(con = con, cdmSchema = "main", writeSchema = "main", cdmName = "my_cdm", cohortTables = c("embarazadas", "valproato"))

# Ruta completa al escritorio en macOS
ruta_escritorio <- file.path("/Users/luciabellas_/Desktop", "VPA.csv")

#calculamos prevalencia por meses 
prevVPA<- estimatePeriodPrevalence(
  cdm = cdm,
  denominatorTable = "embarazadas",
  outcomeTable = "valproato",
  interval = "months",
  minCellCount = 0 )

inciVPAmonthly <- estimateIncidence(
  cdm = cdm,
  denominatorTable = "embarazadas",
  outcomeTable = "valproato",
  interval = "months",
...
write.csv(prevVPA, file = ruta_escritorio)
write.csv(inciVPAmonthly, file = ruta_escritorio)
