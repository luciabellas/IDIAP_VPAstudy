...
lamotrigina <- farmacos %>% 
  filter(grepl("N03AX09", cod)) %>% 
...
DBI::dbWriteTable(con, "lamotrigina", lamotrigina, overwrite = TRUE)
...
cdm <- cdmFromCon(con = con, cdmSchema = "main", writeSchema = "main", cdmName = "my_cdm", cohortTables = c("embarazadas", "lamotrigina"))


# Ruta completa al escritorio en macOS
ruta_escritorio <- file.path("/Users/luciabellas_/Desktop", "LMT.csv")

prevLMT<- estimatePeriodPrevalence(
  cdm = cdm,
  denominatorTable = "embarazadas",
  outcomeTable = "lamotrigina",
  interval = "months",
  minCellCount = 0 )

# Guardar el dataset en formato CSV en el escritorio
write.csv(prevLMT, file = ruta_escritorio)
...
prevLMT$prevalence_start_date <- as.Date(prevLMT$prevalence_start_date)
